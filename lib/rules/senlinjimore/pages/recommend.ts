...
export default function (): HomeRuleItems[] {
  let headers: headers = {
    "x-serverless-sign": "",
    Referer:
  ...
  body["params"] = JSON.stringify({
    ...
    functionArgs: {
      path: "album/get_random_album_list",
      param: {},
      token: "",
      clientInfo: {
        PLATFORM: "mp-weixin",
  ...
  data.forEach((element: any) => {
    items.push({
      ...
      col_type: "pic_1_full",
      url: "hiker://empty/#/" + element._id + `@rule=js:eval(fetch("hiker://files/rules/js/senlinjimore.js"));setHomeResult({data: senlinjimore.imageLists(MY_URL)});`,
    });
    items.push({
        title: "",
